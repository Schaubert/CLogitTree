...
CLogitForest <- function(data,
  ...
  if(is.null(exposure)){
    ...
    ### JW 05.12.2024, changed the calculation from mean of all values to mean
    ### of all corresponding ones
    beta_hat <- tapply(exp.effect, names(exp.effect), mean)
    beta_median_hat <- tapply(exp.effect, names(exp.effect), median)
  }
